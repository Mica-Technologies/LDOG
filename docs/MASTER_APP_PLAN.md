...
>
> Build: `JAVA_HOME=/c/Users/ahawk/.jdks/azul-17.0.18 ./gradlew compileJava` (or `build` for full + tests, or `runClient` for an in-game smoke test). Currently zero compile warnings, 78 unit tests passing. Default windows shell is PowerShell — Bash tool uses bash syntax.

### Where work left off (2026-06-15 — GLSL preprocessor + FSR2 flicker attempt)

- **GLSL preprocessor for active DRAWBUFFERS** (`178b262`) — new `GlslPreprocessor` (`render/shaderpack/`): minimal `#define`/`#undef`/`#ifdef`/`#ifndef`/`#if`/`#elif`/`#else`/`#endif` evaluator + real constant-expression parser (`defined()`, int literals, `! && || == != < > <= >= + - * /`, parens). `ShaderPackRuntime.tryCompile` now strips inactive branches before `parseDrawBuffers`, so the **first surviving** directive is the active one — fixes the old "first textual directive wins" bug that picked dead branches and mis-mapped `gl_FragData[i] → colortex` on real packs (prime suspect for BSL/deferred-pack wrong-looking lighting). We inject no builtin macros, so driver + preprocessor evaluate the same `#define` set and choose the same branch. MRT allocation still over-sizes via `maxDrawBufferIndex` (safe). Covered by `GlslPreprocessorTest` (13 cases). **NEXT real-pack-parity step**: inject OptiFine builtin macros (`MC_VERSION`, `MC_GL_VENDOR_*`, `MC_RENDER_QUALITY`, option defines from `shaders.properties`) so `#if`-gated DRAWBUFFERS match what packs actually intend — both driver source AND this preprocessor need the same injected set.
