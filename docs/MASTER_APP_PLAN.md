...
**Verification status**: compiles clean; the Deferred pack compiles all 3 gbuffer programs + final and activates with no GL/FBO errors at init. The in-world MRT alloc + shadow terrain replay had not yet been visually/runtime-verified at commit time (dev client sat at main menu) — the shadow replay is the highest-risk piece and is opt-in. NEXT SESSION: verify in-world (look for `G-buffer ready`, `Shadow map ready`, no `Shadow terrain pass failed`), then tune shadow matrices/bias visually with a real 1.12.2 pack.

**Still deferred**: multi-WRITE composite chains (composites can read all colortex but only write colortex0); separate shadow frustum (v1 uses camera visibility); entity/TESR shadow casters; sky gbuffer split; custom vertex attrs (mc_Entity, at_tangent); per-dimension recompile on dim change.
