...
>
> Build: `JAVA_HOME=/c/Users/ahawk/.jdks/azul-17.0.18 ./gradlew compileJava` (or `build` for full + tests, or `runClient` for an in-game smoke test). Currently zero compile warnings, 93 unit tests passing. Default windows shell is PowerShell — Bash tool uses bash syntax.

### Where work left off (2026-06-15 cont. — GL error flood fix + BSL parity)

- **Per-frame GL error flood fixed + localized** (`d6db2c8`, `c628fc7`) — real packs emitted ~165 `1282 @ Post render`/s (one uncleared GL error per world frame). New `PipelineGlProbe` drains the GL error queue at each stage boundary (so nothing reaches MC's post-render check) and logs the offending stage **once** per (stage,code) — each boundary's drain clears the queue, so the first stage to report is exactly where the error is born. MC GL errors now **0**. Localized (all `0x502` GL_INVALID_OPERATION, drained, benign): `gbuffer:bind:<category>` (legacy fixed-function + GLSL attribute mixing on the per-object draws), `shadow:depth-render`, and `composite (multi-write)`. The world renders correctly; these are cosmetic driver complaints, not breakage. Fixed the multi-write one's root cause (stale FBO attachment → feedback loop) by detaching all color attachments before each stage. Also: pack programs no longer spam "no active uniform" WARNs (they use only a uniform subset by design — `ShaderProgram.quietMissingUniforms()`).
- **Pack-declared colortex buffer formats** (`caaf2ae`, `60940e3`) — advanced packs declare `const int colortexNFormat = R11F_G11F_B10F/RGBA16/...;` (+ legacy `gauxN` = colortex4..7). We were allocating every aux as RGBA8, clipping HDR buffers (BSL "dark/washed") and losing 16-bit precision. New `ShaderColortexFormats` parses them → GL internal formats; `ShaderPackGbufferManager` (aux attachments) AND the multi-write composite scratch (`mrtCur/mrtAlt`) now allocate with the declared format. colortex0 stays the shared scene texture (already RGBA16F ⊇ R11F_G11F_B10F under the HDR pipeline). Unit-tested (`ShaderColortexFormatsTest`, 7 cases).
- **BSL v8.2 + v10.1 are 1.12.2-compatible** — both are `#version 120` + `world0/` + `/program/` includes + multi-branch `#ifdef` DRAWBUFFERS. The earlier "GLSL-330-core, fundamentally incompatible" note was WRONG for these specific 1.12.2 builds (it conflated them with the 1.16 versions). With preprocessor + macros + formats now in, BSL should render far better — **needs in-world test** (run with HDR pipeline ON so colortex0 HDR matters). Remaining BSL-specific gaps if it still looks off: custom textures (`texture.noise`, `texture.composite.depthtex2=dirt.png`), per-program `alphaTest.gbuffers_*`, colortex8/9 (only used with MULTICOLORED_BLOCKLIGHT, off by default), and shader-options GUI (runs at defaults).

### Where work left off (2026-06-15 — GLSL preprocessor + FSR2 flicker attempt)

- **GLSL preprocessor for active DRAWBUFFERS** (`178b262`) — new `GlslPreprocessor` (`render/shaderpack/`): minimal `#define`/`#undef`/`#ifdef`/`#ifndef`/`#if`/`#elif`/`#else`/`#endif` evaluator + real constant-expression parser (`defined()`, int literals, `! && || == != < > <= >= + - * /`, parens). `ShaderPackRuntime.tryCompile` now strips inactive branches before `parseDrawBuffers`, so the **first surviving** directive is the active one — fixes the old "first textual directive wins" bug that picked dead branches and mis-mapped `gl_FragData[i] → colortex` on real packs (prime suspect for BSL/deferred-pack wrong-looking lighting). MRT allocation still over-sizes via `maxDrawBufferIndex` (safe). Covered by `GlslPreprocessorTest` (13 cases).
- **OptiFine builtin macro injection** (`e337536`) — new `ShaderMacros` builds OF's standard `MC_*` defines from the live GL context + OS (`MC_VERSION=11202`, `MC_GL_VERSION`, `MC_GLSL_VERSION`, `MC_OS_*`, `MC_GL_VENDOR_*`, `MC_GL_RENDERER_*`, `MC_RENDER_QUALITY/SHADOW_QUALITY/HAND_DEPTH`) and injects them as `#define`s after `#version`. The **same** map seeds `GlslPreprocessor`, so DRAWBUFFERS resolution sees exactly what the driver compiles — version/vendor-gated MRT sets now resolve to the real 1.12.2/this-GPU branch (modern `MC_GL_VERSION >= 300` paths correctly switch OFF instead of by accident). Deliberately omits `MC_NORMAL_MAP`/`MC_SPECULAR_MAP` (those samplers read black → packs take their no-map path). Pure helpers unit-tested (`ShaderMacrosTest`, 8 cases). **Still NOT injected**: pack option overrides from `shaders.properties`/a shader-options GUI — in-source `#define` defaults already work (driver + preprocessor both honour them), so packs render at default settings; a user-facing option GUI is the remaining piece.
- **Uniform parity** (`d97568a`) — added `previousCameraPosition` (motion-blur/temporal packs) + `frameTimeCounter` now wraps at 3600s like OF. Additive for real packs; built-ins reference neither.
- **FSR2 in-motion flicker — STILL UNRESOLVED.** User config: `FSR2 + TAA off + internalRenderScale 0.75 + HDR pipeline`, flickers while flying *even with no shader pack*. Attempts that did NOT fix it: history-weight 0 (`46e04d0`), TAA-only jitter + axis-snapped normals (`0a65c42`), bilinear-blit-when-TAA-off to kill Lanczos ringing (`f1f7669`). Since bilinear (no negative lobes) still flickers, the cause is **not** the FSR2 resolve kernel — that path is exhausted. Deferred to a fresh session: instrument properly (test `internalRenderScale=1.0` to rule out the scaled-render path; disable post pipeline entirely to confirm pipeline-vs-vanilla; then bisect features). Do NOT keep blind-guessing FSR2.
